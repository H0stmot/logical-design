module pre_res #(
    ...
    input  wire [EXP_W+MANT_W:0] op_a,          // Операнд A
    input  wire [EXP_W+MANT_W:0] op_b,          // Операнд B
    input  wire [3:0]            special_case,  // Вектор признаков из модуля анализа:
                                                 // [3] - case 1: NaN
                                                 // [2] - case 2: 0 * inf
                                                 // [1] - case 3: 0 * число
                                                 // [0] - case 4: inf * число
    output wire [EXP_W+MANT_W:0] result         // Результат
);

    localparam WIDTH = EXP_W + MANT_W + 1;

    // Извлечение компонентов операндов
    wire sign_a = op_a[WIDTH-1];
    wire sign_b = op_b[WIDTH-1];
    ...
    wire [WIDTH-1:0] case1_result = {
        op_a[WIDTH-1],        // sign
        op_a[WIDTH-2:MANT_W], // exponent
        1'b1, op_a[MANT_W-2:0] // mantissa со старшим битом = 1
    };

    // Case 2: 0 * inf - s=1, exp=все_единицы, frac=1_0...0
    wire [WIDTH-1:0] case2_result = {
