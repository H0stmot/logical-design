module rounding_module #(
    parameter MANT_W = 23             // Ширина мантиссы результата (без скрытого бита)
)(
    input  wire [1:0]          round_mode,        // Режим округления
    input  wire [2*MANT_W+1:0] input_value,       // Входное значение (TOTAL_WIDTH = 2*MANT_W+2)
    input  wire                sign_bit,          // Знаковый бит (отдельный вход)
    output wire [MANT_W-1:0]   rounded,           // Округленное значение
    ...
    localparam HIGH_PART_WIDTH = MANT_W;
    localparam LOW_PART_WIDTH  = MANT_W + 1;
    localparam TOTAL_WIDTH     = 2*MANT_W + 2;
